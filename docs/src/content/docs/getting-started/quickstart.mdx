...
<Aside type="note">
  The `.env` file is required by Docker Compose. It sets PostgreSQL credentials (`POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`), the Redis password (`REDIS_PASSWORD`), and the server timezone (`TZ`). Copy `docker/.env.example` to `docker/.env` and edit as needed.
</Aside>
